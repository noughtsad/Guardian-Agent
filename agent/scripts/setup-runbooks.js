...
const runbookDir = path.resolve('/tmp/runbooks');
...
for (const [name, content] of runbooks) {
  fs.writeFileSync(path.join(runbookDir, name), content, 'utf8');
}

console.log(`Created ${runbooks.size} runbooks in ${runbookDir}`);
for (const file of fs.readdirSync(runbookDir)) {
  console.log(file);
}
